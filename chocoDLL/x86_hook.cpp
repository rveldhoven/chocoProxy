...
uint32_t dword_abs(uint32_t a, uint32_t b)
{
	return a < b ? b - a : a - b;
}

bool cx86PrologueHook::_hook_is_code_cave_valid()
{
	return true;
}

void cx86PrologueHook::_hook_set_trampoline()
{
...
void cx86PrologueHook::_hook_set_hook()
{
...
cx86PrologueHook::cx86PrologueHook(std::shared_ptr<CopyPatterns> patterns)
...
cx86PrologueHook::~cx86PrologueHook()
{
	if (is_hook_set() == true)
		hook_unset();
}

void cx86PrologueHook::hook_set_trampoline(uint32_t hook_location, uint32_t code_cave, uint32_t hook_destination, bool call_type)
{
...
void cx86PrologueHook::hook_set_hook()
{
...
void cx86PrologueHook::hook_set_all(uint32_t hook_location, uint32_t code_cave, uint32_t hook_destination, bool call_type)
{
...
void cx86PrologueHook::hook_unset()
{
	DWORD Dummy = 0, Dummy2 = 0;
...
uint32_t cx86PrologueHook::hook_get_trampoline_end()
{
	return _hook_trampoline_end;
}

bool cx86PrologueHook::is_hook_set()
{
	return _hook_is_set;
}

cx86CodeCave::cx86CodeCave(uint32_t abase_address, uint32_t alength)
	: base_address(abase_address), current_address(abase_address), length(alength)
{
}

std::shared_ptr<cx86CodeCave> cx86HookManager::_allocate_code_cave_in_range(uint32_t hook_location)
{
	SYSTEM_INFO SI = {};
	...
	uint32_t code_cave_location = hook_location + SI.dwPageSize;
	uint32_t code_cave_length = SI.dwPageSize * 8;

	while (allocation_success == false)
	{
		uint32_t real_cave_location = (uint32_t)VirtualAlloc((void*)code_cave_location, code_cave_length, MEM_COMMIT | MEM_RESERVE, 0x40);
	...
	auto allocated_cave = std::make_shared<cx86CodeCave>(code_cave_location, code_cave_length);
...
std::shared_ptr<cx86CodeCave> cx86HookManager::_get_cave_in_range(uint32_t hook_location)
{
	for (auto& cave : _code_cave_locations)
	{
		if (dword_abs(cave.second->base_address + cave.second->length, cave.second->current_address) <= 0xf0)
			continue;
...
cx86HookManager::cx86HookManager(std::shared_ptr<CopyPatterns> patterns)
	: _copy_patterns(patterns)
{
}

cx86HookManager::~cx86HookManager()
{
...
std::shared_ptr<cx86PrologueHook> cx86HookManager::set_hook_all(uint32_t hook_location, uint32_t hook_destination, bool call_type)
{
	...
	auto hook = std::make_shared<cx86PrologueHook>(_copy_patterns);
...
std::shared_ptr<cx86PrologueHook> cx86HookManager::set_hook_trampoline(uint32_t hook_location, uint32_t hook_destination, bool call_type)
{
	auto code_cave = _get_cave_in_range(hook_location);
