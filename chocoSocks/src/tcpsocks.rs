...
pub fn handle_tcp_client(mut client_stream : TcpStream, mut global_state : globalState) 
{
	...
	*/
	...
	let state_data = streamState::new( // dummy data
	...
	loop 
	{
		...
		if bytes_received != 0
		{
			save_to_pcap(&packet_data[0..bytes_received].to_vec(), &1, &0, &server_client_syn, &client_server_syn, &mut file);
		...
		if bytes_received != 0
		{
			save_to_pcap(&packet_data[0..bytes_received].to_vec(), &0, &1, &client_server_syn, &server_client_syn, &mut file);
